...
def get_api(name: str, max_size: str) -> str:
    response, response1, data_gender, data_cats = get_url(name, max_size)
...
def login(username_entry: EntryWithBuffer, password_entry: EntryWithBuffer, root: CTk.CTk, is_protected: bool) -> None:
    username, password, user = login1(username_entry,  password_entry)
...
def login1(username_entry: EntryWithBuffer, password_entry: EntryWithBuffer):
...
def open_user_window(username: str, root: CTk.CTk, is_protected: bool) -> None:
    ...
    open_user_window2(user_window, frame, username, root, is_protected, entry_name)


def authenticate2():
    return None

def open_user_window2(user_window, frame, username: str, root: CTk.CTk, is_protected: bool, entry_name) -> None:
    ...
    btn_result = CTk.CTkButton(master=frame, text="GO", width=330, height=50, hover_color="PaleGreen3",
                               text_color="black", corner_radius=20, command=lambda: show_api(entry_name, entry_size, frame,
                                                                                              is_protected))
...
def get_url(name: str, max_size: str):
    ...
    return response, response1,data_gender, data_cats
